...
class ChatService {
    async chat(conversationId, question, userId) {
        ...
        const userMessage = await messageService.createMessage(conversationId, "USER", question, [], user?.name);
...
9. Give response in Markdown`;
        ...
        const assistantMessage = await messageService.createMessage(conversation.id, "ASSISTANT", response, chunks.map((chunk) => chunk.id), "Addax");
